...
import requests
import models
from identity import validate_mobile_number
import logging
import config
...
async def root(status=status.HTTP_200_OK):
    return {"Welcome To Luco Pay": "True"}
...
async def msisdn_identity(identity: models.Identity):
    if not identity.msisdn:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="msisdn is required"
        )
    ...
    if result:
        ...
        raise HTTPException(
            ...
            detail="Failed to validate mobile number",
        )
...
async def payment_webhook(reference: models.TransReference):
    ...
    try:
        ...
        logger.info(
            f"Checking transaction at: {config.API_BASE_URL}/check_transaction_status"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching transaction: {str(e)}"
        )
