...
def generate_order_id(size=8, chars=string.ascii_uppercase + string.digits):
    ...
    random_part = ''.join(choice(chars) for _ in range(size))
    return f"LXN{random_part}"



def id_generator(size=2, chars='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'):
    return 'LXN'.join(choice(chars) for _ in range(size))
...
if __name__ == "__main__":
    ...
    secure_result = generate_secure_refer_id(size=8, secret_key=secret_key)
